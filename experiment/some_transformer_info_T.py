#from model.simple_Ttrans import simple
...
class experiment(object):
    ...
    def _get_data(self,tt_ratio,normalize,cross_start=False):
        ...
        if cross_start:
            train_input=np.delete(input, np.s_[cross_start,cross_start+60],axis=0)
            train_output=np.delete(output, np.s_[cross_start,cross_start+60],axis=0)
            test_input=input[cross_start:cross_start+60]
            test_output=output[cross_start:cross_start+60]
    ...
    def _get_bucket_data(self,normalize,cross_start=False):
        train_input,test_input,train_output,test_output=[],[],[],[]
        input=np.load('./data/input_18month_imputed_18prototype.npy',allow_pickle=True)
        # input=np.load('./data/input_15month_no_nan_withprototype_holiday.npy',allow_pickle=True)

        dates=np.load('./data/selected_days.npy',allow_pickle=True)
        ...
        train_input=train_input[:,:,[4,5,6,7,8,9,10]].astype(np.float)
        test_input=test_input[:,:,[4,5,6,7,8,9,10]].astype(np.float)

        train_input[:,:,4]=train_input[:,:,4]/50
        test_input[:,:,4]=test_input[:,:,4]/50
        ...
        for epoch in range(self.epochs):
            ...
            with torch.no_grad():
                ...
                if t_loss/i<bestloss:
                    bestloss=t_loss/i
                    torch.save(self.model.state_dict(),'./checkpoints/simple_trans_18m_newprototype_bucket.model')

    def validate(self):
        self.model.load_state_dict(torch.load('./checkpoints/simple_trans_18m_newprototype_bucket.model'))
        lossf=nn.MSELoss().cuda()
        l1=nn.L1Loss().cuda()
        ...
        with torch.no_grad():
            ...
            d=np.asarray(d)
            print(e.shape)
            np.save('bucket_error18.npy',e)
            np.save('bucket_dinfo18.npy',d)
            print('saved')
            print('w_f=')
            print(w_f)
            print('w_t=')
    ...
    def cross_validation(self):
        llist=[]
        # for i in range(0,6):
        #     self._get_data(0.1,False,cross_start=i*60)
        #     self.model = self._build_model().cuda()
        #     self.train()
        #     e,d,l,w_f,w_t=self.validate()
        #     np.save('cross_error18_'+str(i)+'.npy',e)
        #     np.save('cross_dinfo18_'+str(i)+'.npy',d)
        #     np.save('w_f18_'+str(i)+'.npy',w_f)
        #     np.save('w_t18_'+str(i)+'.npy',w_t)
        #     llist.append(l)

        self._get_data(0.1,False,cross_start=1)
        self.model = self._build_model().cuda()
        self.train()
        e,d,l,w_f,w_t=self.validate()
        np.save('cross_error18_'+str(0)+'.npy',e)
        np.save('cross_dinfo18_'+str(0)+'.npy',d)
        np.save('w_f18_'+str(0)+'.npy',w_f)
        np.save('w_t18_'+str(0)+'.npy',w_t)
        llist.append(l)
        print(llist)
