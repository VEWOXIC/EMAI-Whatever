...
class SCI_Point_Mask(nn.Module):
    ...
    def forward(self, x):
        ...
        x_covariate = x[...,1::]
        point_processed_x = torch.zeros(b, l, 1, device=x.device)

        point_mask_x, point_mask = self.creatMaskEvenSplit1(x_value, part = self.point_part)
        ...
        for i in range(self.point_part):
            ...
            masked_input = torch.cat((point_mask_x[i],x_covariate),dim=2)
            mask_x_process = self.blocks[0](masked_input)[...,0]
            mask_x_process = mask_x_process.unsqueeze(2)
...
if __name__ == '__main__':
    ...
    model = SCI_Point_Mask(args, num_classes=96, input_len=96, input_dim=1,
                   number_levels=len(part),
                   number_level_part=part, num_layers=3, concat_len=None).cuda()
    x = torch.randn(1, 96, 1).cuda()
    y = model(x)
    print(y.shape)
