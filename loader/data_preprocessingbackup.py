...
def read_test_csv(test_csv=None):
    if test_csv:
        csv_file = open(test_csv, 'r', encoding='utf-8', newline='')
    else:
...
def read_train_csv(train_csv=None):
    if train_csv:
        csv_file = open(train_csv, 'r', encoding='utf-8', newline='')
    else:
...
def test_data_preprocessing(prototype,test_csv=None):
    s = read_test_csv(test_csv)
...
def get_day_index(years,dayofweek,dates,yearmean=True):
...
def train_input_output(training_file=None):
    input_array1 = read_train_csv(training_file)
    everyday_a = []
    total_a = []
...
if __name__=='__main__':
    ...
    print(train_input==input[3000])
    print(input[0,0],train_input[0,0])
